...
class PlanQASection: PlanTableViewSectionType {
    
    let type: PlanType
    
    var model: PlanQAModel?
    ...
    func loadData(callback: @escaping ErrorTask) {
        PlanAPI.otherApi(params: ["type": type.rawValue]).reqeust { [weak self] in
            ...
            self.model = PlanQAModel(demoImageNamed: "plan_\(self.type.rawValue)_qa")
        }
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.ns.dequeueCell(PlanQACell.self, for: indexPath)
