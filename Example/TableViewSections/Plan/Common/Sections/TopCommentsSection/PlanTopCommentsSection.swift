...
class PlanTopCommentsSection: PlanTableViewSectionType {
    
    let type: PlanType
    
    var model: PlanTopCommentsModel?
    ...
    func loadData(callback: @escaping ErrorTask) {
        PlanAPI.otherApi(params: ["type": type.rawValue]).reqeust { [weak self] in
            ...
            self.model = PlanTopCommentsModel(demoImageNamed: "plan_\(self.type.rawValue)_top_comments")
        }
    }
