...
class PlanProfitRatioSection: PlanTableViewSectionType {
    
    let type: PlanType
    
    var model: PlanProfitRatioModel?
    ...
    func loadData(callback: @escaping ErrorTask) {
        PlanAPI.otherApi(params: ["type": type.rawValue]).reqeust { [weak self] in
            ...
            self.model = PlanProfitRatioModel(demoImageNamed: "plan_\(self.type.rawValue)_profit_ratio")
        }
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.ns.dequeueCell(PlanProfitRatioCell.self, for: indexPath)
