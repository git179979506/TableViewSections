...
class PlanHoldingSection: PlanTableViewSectionType {
    
    let type: PlanType
    
    var model: PlanHoldingModel?
    ...
    func loadData(callback: @escaping ErrorTask) {
        PlanAPI.otherApi(params: ["type": type.rawValue]).reqeust { [weak self] in
            ...
            self.model = PlanHoldingModel(demoImageNamed: "plan_\(self.type.rawValue)_holding")
        }
    }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.ns.dequeueCell(PlanHoldingCell.self, for: indexPath)
